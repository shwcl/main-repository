...
<?php 
	...
	if (isset($_GET['p']) && is_numeric($_GET['p'])) {
		...
		$q = 'SELECT count(unit_id) FROM unit';
	...
	//run the query
	$q = "SELECT unit_id, unit_name
	FROM unit 
	ORDER BY unit_name ASC LIMIT $start, $display";
	...
	if ($r) {
		...
		//create table header
		echo '<table class="table1" width="65%"><tr><td>Department ID</td><td>Department Name</td><td>Edit</td></tr>';
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			...
			echo '<tr bgcolor="' . $bg . ' "><td>'. $row['unit_id'].'</td><td>'.$row['unit_name'].'</td>
			<td><a href="editunit.php?id='.$row['unit_id'].'">Edit</a></td></tr>';
		}
		...
		echo '<p>The data could not be retrieved.</p>';
	...
	if ($no_pages > 1) {
		...
		If ($current_page != 1) {
			echo '<a href="viewunits.php?s=' . ($start - $display) . '&p=' . $no_pages . '">Previous</a> ';
		} 
		...
		for ($i = 1; $i <= $no_pages; $i++) {
			if ($i != $current_page) {
				...
				echo '<a href="viewunits.php?s=' . $display * ($i - 1) . '&p=' . $no_pages . '">' . $i . '</a> ';
		...
		If ($current_page != $no_pages) {
			echo '<a href="viewunits.php?s=' . ($start + $display) . '&p=' . $no_pages . '">Next</a> ';
