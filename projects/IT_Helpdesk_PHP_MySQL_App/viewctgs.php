...
<?php 
	...
	if ($r) {
		...
		//create table header
		echo '<table class="table1" width="65%"><tr><td>category_id</td><td>Category name</td><td>Edit</td></tr>';
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			...
			echo '<tr bgcolor="' . $bg . ' "><td>'. $row['category_id'].'</td><td>'.$row['category_name'].'</td><td>
			<td><a href="editctg.php?id='.$row['category_id'].'">Edit</a></td></tr>';
		}
	...
	if ($no_pages > 1) {
		...
		If ($current_page != 1) {
			echo '<a href="viewctgs.php?s=' . ($start - $display) . '&p=' . $no_pages . '">Previous</a> ';
		} 
		...
		for ($i = 1; $i <= $no_pages; $i++) {
			if ($i != $current_page) {
				...
				echo '<a href="viewctgs.php?s=' . $display * ($i - 1) . '&p=' . $no_pages . '">' . $i . '</a> ';
		...
		If ($current_page != $no_pages) {
			echo '<a href="viewctgs.php?s=' . ($start + $display) . '&p=' . $no_pages . '">Next</a> ';
